...
- **Experiment E1 (build hour one) — amended 2026-08-07 after read-only probing:** the kernel *requires* a hook (`hook = 0` reverts `HookRequired()`), and the router-hook reverts `fund()` (selector `0x32d53d69`) for any flow it doesn't govern — so "skip the router" is impossible. Settled read-only already: `complete()` is strictly evaluator-only, and `createJob` accepts an arbitrary EOA evaluator. E1-revised asks the remaining question: *can a Docket EOA in the `evaluator` slot `complete()` a properly router-registered, funded, submitted job?* — answerable by `eth_simulateV1` with state overrides (the technique that found the blocker), likely with zero funding. If yes: accept-is-settlement is instant and mainnet's 7-day window stops being a UX constraint. If no: testnet (1-day window) for judge-facing escrow hires; mainnet demo ends at SUBMITTED + verified deliverable with settlement framed as scheduled.
