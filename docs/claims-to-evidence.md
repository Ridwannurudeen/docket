...
| C-04 | The v3 paired report has three stage-one specifications, but no input is locked and no arm has run. | Range hashes `0xcfaf6d…68bd6` / `0x2d3a61…0d282`; Yield `0x2d567e…54ace` / `0x9455cc…baa1e`; Warden `0x919b37…fb4bf` / `0x27ce69…db01b`; every `inputs_sha256=""`; input/run artifacts **missing**. | V3 has no API report. This says nothing about the outcome of a future run. |
