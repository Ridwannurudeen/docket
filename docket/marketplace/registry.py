...
SERVICES: dict[str, ServiceRecord] = {
    "health-guard": ServiceRecord(
        ...
        limitations=(
            ...
            "read at the same block. Docket never holds a key to your wallet and cannot "
            "sign for it. Prepared calls are yours to sign unless you grant a session key — "
            "created by Docket, bounded on chain by a contract allowlist, a per-token cap, "
            "a gas ceiling and an expiry you set, and revocable by you at any time, which "
            "sweeps its balance back to you. Docket's own checks sit in front of the "
            "chain's and may only ever refuse more, never less. Your borrower account is "
            "never transferred to Docket and nothing is ever custodied on your behalf: a "
            "repay is repayBorrowBehalf, which pays your debt down from the session's own "
            "tokens and moves nothing of yours. Supplying collateral has no on-behalf form "
            "at all — mint credits whoever calls it — so a collateral add is always yours "
            "to sign and a session is never permitted to send one. Remedies are repay and "
            "supply-collateral only; borrowing and withdrawing are not encoded and no "
            "argument produces one. Every approval is for an exact amount and never "
            "unlimited. Venus caps a repay at what is owed rather than reverting and "
            "signals failure by returning an error code rather than reverting, so a mined "
            "transaction is not by itself evidence of how much was retired; that is read "
            "from the next account snapshot. A remedy changes a balance Venus holds at the "
            "block it lands in and establishes nothing about a liquidation that did not "
            "happen, because that is a claim about a world nobody observed. A single "
    ...
    "range-doctor": ServiceRecord(
        ...
        limitations=(
            "PancakeSwap v3 on BSC only. Docket never holds a key to your wallet and cannot "
            "sign for it. A one-off hire reads and states what it read; a persistent watch "
            "adds prepared calls, and those calls are yours to sign unless you grant a "
            "session key — created by Docket, bounded on chain by a contract allowlist, a "
            "per-token cap, a gas ceiling and an expiry you set, and revocable by you at "
            "any time, which sweeps its balance back to you. Docket's own checks sit in "
            "front of the chain's and may only ever refuse more, never less. The ERC-721 "
            "approval over your position NFT is yours to sign because only its holder can "
            "make it, the replacement position is minted to your address, and no position "
            "NFT is ever transferred to Docket. The session holds the two tokens for the "
            "few blocks between closing the old position and minting the new one, and "
            "nothing longer. Resetting a range realises impermanent loss that was "
            "unrealised while the position sat still, and that loss is not in the figures "
            "the decision was made on. It reads ticks rather than prices, and a position's "
            "uncollected fees come from tokensOwed, which is stale until something touches "
            "the position — so that figure can read low. Time outside a range is only ever "
            "the time between observations that saw it outside; a departure between two "
            "reads is not dated. Fee projections annualise one pool-wide 24-hour "
            "observation and are not a forecast or this position's measured earnings. A "
            "confirmed transaction shows the reset executed as it was authorised and shows "
            "nothing about whether it was worth taking. One read covers a bounded slice of "
            "a wallet's positions, newest first, and returns positions_held beside "
            "positions_examined so a truncated read announces itself; closed positions are "
            "counted and not detailed. A single recorded read; no paired run against a "
            "person stands behind the metrics on this card."
        ),
    ),
