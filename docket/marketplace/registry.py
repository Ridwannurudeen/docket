...
SERVICES: dict[str, ServiceRecord] = {
    "health-guard": ServiceRecord(
        ...
        limitations=(
            ...
            "Docket never holds a key to your wallet and cannot sign for it. Prepared calls are yours to sign unless you grant a session key. That key is created by Docket and bounded by a contract allowlist, a per-token cap, a gas ceiling and an expiry you set — but those are checks Docket runs in front of every send, not rules a chain enforces, so what actually caps your exposure is how much you fund the session with. Fund it with what you are willing to lose to a bug. Revoking it sweeps its balance back to you at any time. Your borrower account is never transferred to Docket and nothing is custodied on your behalf: a repay is repayBorrowBehalf, which pays your debt down out of the session's own tokens and moves nothing of yours. Supplying collateral has no on-behalf form at all — mint credits whoever calls it — so a collateral add is always yours to sign, is reported rather than offered for execution, and a session is never permitted to send one. Every approval is for an exact amount and never unlimited. The approval is put to the chain before it is offered; the call that spends under it is marked as waiting on that approval rather than simulated against a state that does not exist, and every call is simulated again from its real sender immediately before it is sent."
    ...
    "range-doctor": ServiceRecord(
        ...
        limitations=(
            ...
            "Docket never holds a key to your wallet and cannot sign for it. A one-off hire reads and states what it read; a persistent watch adds prepared calls, and those calls are yours to sign unless you grant a session key. That key is created by Docket and bounded by a contract allowlist, a per-token cap, a gas ceiling and an expiry you set — but be exact about what those bounds are: they are checks Docket runs in front of every send, not rules a chain enforces, so what actually caps your exposure is how much you fund the session with. Fund it with what you are willing to lose to a bug. Revoking it sweeps its balance back to you at any time. The ERC-721 approval over your position NFT is yours to make because only its holder can, Docket asks for it rather than drafting it, the replacement position is minted to your address, and no position NFT is ever transferred to Docket. The session holds the two tokens between closing the old position and minting the new one; fee residue swept by the collect and any surplus the swap leaves above the mint's floor stay there until you revoke. Each call is put to the chain before it is offered where its preconditions already hold; the swap and the mint spend tokens the collect has not released yet, so they are marked as waiting on it rather than simulated against a state that does not exist — and every call is simulated again from its real sender immediately before it is sent. Resetting a range realises impermanent loss that was unrealised while the position sat still, and that loss is not in the figures the decision was made on."
