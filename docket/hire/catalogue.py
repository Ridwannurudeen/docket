...
SERVICES: dict[str, Service] = {
    ...
    "grid-operator": Service(
        ...
        what_you_get=(
            ...
            "strategy is not a decision about the money. Revoke requests a sweep of the "
            "session back to the owner; the runner also starts that path after expiry. "
            "The shutdown threshold is Docket's own observation and not a stop "
            "order held by a venue: it retires the remaining levels and sells nothing. "
            "Acting needs an owner-authorized, separately funded session EOA. "
            "Docket's runner holds the session key, never the owner's wallet key. "
            "Spend caps, call allowlists and expiry are off-chain checks before each "
            "send, not an on-chain session validator. The funded float is at risk if "
            "the runner or session key is compromised; fund only what you can afford "
            "to lose. Revocation and expiry rely on the runner to sweep the remaining "
            "balance, and close only after balance reads return zero. Every figure "
            "comes back with the block it was read "
