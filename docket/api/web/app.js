...
function browseRow(item, responders) {
  ...
  const name = item.name ? item.name : `Agent #${item.token_id}`;
...
async function workedExample(currentId) {
  ...
  if (!answered) return "";
  const name = detail.name ? detail.name : `Agent #${detail.token_id}`;
...
async function initAgent() {
  ...
  try {
    ...
    document.title = `${detail.name ? detail.name : `Agent #${detail.token_id}`} — Docket`;
