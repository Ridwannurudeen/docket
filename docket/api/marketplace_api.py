...
from ..scan8004 import Scan8004Client, lookup_owner_onchain
...
def _token_id(agent_id: str) -> str | None:
    """The token id inside a `chain:registry:token` agent id, or the bare id itself."""
    parts = agent_id.split(":")
    candidate = parts[-1] if len(parts) == 3 else agent_id
...
def marketplace_router(context: MarketplaceContext) -> APIRouter:
    ...
    def hydrate(rows: list[dict]) -> list[dict]:
        """Cache one page of registry results as listings and return them.
        ...
        for row in rows:
            ...
            if existing:
                ...
                continue
            listing = listing_from_registry(row, chain_id=context.chain_id)
    ...
    async def get_listing(request: Request, agent_id: str):
        ...
        if not listing:
            token_id = _token_id(agent_id)
            if token_id is None:
                return _error(
                    404,
                    "listing_not_found",
                    f"No listing for {agent_id!r}, and it is not a token id Docket can "
                    "look up. Search at GET /api/agents?q=.",
                )
            resets_in = _spend_lookup(context.lookups, client_ip(request))
            if resets_in is not None:
                return _error(
